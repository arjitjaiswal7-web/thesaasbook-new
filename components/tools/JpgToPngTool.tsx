...
type OutputMode = "optimized" | "lossless";
type ColorHandling = "preserve" | "palette-256" | "palette-128";
...
function getColorCount(outputMode: OutputMode, colorHandling: ColorHandling) {
  if (outputMode === "lossless") {
    return 0;
  }

  if (colorHandling === "palette-256") {
    return 256;
  }

  if (colorHandling === "palette-128") {
    return 128;
  }
...
async function convertJpgToPng(
  ...
  if (outputMode === "lossless") {
    ...
    const encoded = UPNG.encode(
      ...
      canvas.width,
      canvas.height,
      getColorCount(outputMode, colorHandling),
    );
    blob = new Blob([encoded], { type: "image/png" });
  }

  if (!blob) {
    throw new Error("JPG to PNG conversion failed for one of the files.");
  }
...
export default function JpgToPngTool() {
  ...
  const [colorHandling, setColorHandling] = useState<ColorHandling>("palette-256");
  ...
  const summary = useMemo(() => {
    ...
    const delta = original > 0 ? ((converted - original) / original) * 100 : 0;

    return { original, converted, delta };
  }, [results]);

  const settingsNote =
    outputMode === "lossless"
      ? "Lossless PNG keeps all visible colors. File size can grow compared with JPG because PNG does not use JPG-style lossy compression."
      : colorHandling === "preserve"
        ? "Optimized PNG removes metadata and uses a tighter PNG encoder while preserving visible colors."
        : colorHandling === "palette-256"
          ? "Optimized PNG reduces the palette to around 256 colors to keep files smaller without adding filters or visual effects."
          : "Optimized PNG reduces the palette further for smaller files. This can slightly reduce subtle color variation, but no filter is added.";
  ...
  const handleConvert = async () => {
    ...
    try {
      ...
      for (const [index, item] of uploads.entries()) {
        const converted = await convertJpgToPng(item.file, outputMode, colorHandling);
      ...
      setError(
        conversionError instanceof Error
          ...
          : "JPG to PNG conversion failed. Please try again.",
      );
    } finally {
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="rounded-[1.5rem] border border-slate-200 bg-slate-50 p-4 sm:p-5">
        <div className="grid gap-4 lg:grid-cols-[1fr_1fr_auto]">
          <label className="block text-sm font-medium text-slate-700">
            ...
            >
              <option value="optimized">Optimized PNG</option>
          ...
          <label className="block text-sm font-medium text-slate-700">
            ...
            <select
              value={colorHandling}
              onChange={(event) => setColorHandling(event.target.value as ColorHandling)}
              disabled={outputMode === "lossless"}
              className="mt-2 w-full rounded-xl border border-slate-200 bg-white px-3 py-2.5 text-sm text-slate-900 disabled:cursor-not-allowed disabled:bg-slate-100 disabled:text-slate-500"
            >
              <option value="preserve">Preserve original colors</option>
              <option value="palette-256">Smaller file (256 colors)</option>
              <option value="palette-128">Smaller file (128 colors)</option>
            </select>
          </label>

          <div className="rounded-xl border border-slate-200 bg-white px-4 py-3 text-sm text-slate-600 lg:self-end">
            No filter or visual effect is applied.
          </div>
        </div>

        <p className="mt-3 text-xs leading-5 text-slate-500">{settingsNote}</p>
      </div>
      ...
      >
        ...
        <p className="mt-2 text-sm text-slate-600">Batch conversion supported. Optimized PNG is recommended for smaller files.</p>
        <button
          type="button"
        ...
        <div className="rounded-[1.5rem] border border-slate-200 bg-white p-4 sm:p-5">
          <div className="flex flex-wrap items-center justify-between gap-3">
            ...
            <div className="flex items-center gap-2">
              ...
              >
                {isConverting ? <Loader2 className="h-4 w-4 animate-spin" aria-hidden="true" /> : <ImageIcon className="h-4 w-4" aria-hidden="true" />}
                    ...
                    {result ? (
                      <>
                        <p className="text-xs text-slate-500">
                          PNG: {result.width} x {result.height}
                        </p>
                        <p className="text-xs text-slate-500">Output: {bytesToSize(result.convertedSize)}</p>
                        <p className={`text-xs font-semibold ${result.convertedSize <= result.originalSize ? "text-emerald-700" : "text-amber-700"}`}>
                          {result.convertedSize <= result.originalSize
                            ? `Saved ${(((result.originalSize - result.convertedSize) / result.originalSize) * 100).toFixed(1)}%`
                            : `Larger by ${(((result.convertedSize - result.originalSize) / result.originalSize) * 100).toFixed(1)}%`}
                        </p>
                        <button
                          type="button"
      ...
      {results.length ? (
        <div className="rounded-[1.5rem] border border-sky-200 bg-sky-50/60 p-4 sm:p-5">
          <p className="text-sm font-semibold text-sky-900">Conversion summary</p>
          <p className="mt-2 text-sm text-sky-800">
            {bytesToSize(summary.original)} to {bytesToSize(summary.converted)} ({summary.delta >= 0 ? "+" : ""}{summary.delta.toFixed(1)}% size change)
          </p>
        </div>
      ) : null}
